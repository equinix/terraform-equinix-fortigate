...
variable "hostname" {
  ...
  type        = string
  validation {
    condition     = length(var.hostname) >= 2 && length(var.hostname) <= 50
...
variable "interface_count" {
  ...
  default     = 0
  validation {
    condition     = var.interface_count == 0 || var.interface_count == 10 || var.interface_count == 18
...
variable "secondary" {
  ...
  validation {
    ...
    error_message = "Key 'hostname' has to be between 2 and 50 characters long."
  }
  validation {
